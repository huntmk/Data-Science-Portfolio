...
total_cases_north_of_equator = confirmed %>% 
  filter(Lat > 0) %>% 
  select(`3/22/20`) %>% 
  sum()
...
transposed_confirmed = transposed_confirmed %>%
...
transposed_confirmed = apply(transposed_confirmed, 2, as.numeric) %>%
  as.data.frame()
head(transposed_confirmed)
```
...
# Add the date column
transposed_confirmed = transposed_confirmed %>%
  mutate(date = c(1:nrow(transposed_confirmed)))
